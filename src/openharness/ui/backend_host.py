...
class ReactBackendHost:
    ...
    async def run(self) -> int:
        ...
        try:
            while self._running:
                ...
                if not line:
                    continue
                attachments = [str(path) for path in (request.attachments or []) if str(path).strip()]
                self._busy = True
                try:
                    should_continue = await self._process_line(line, attachments=attachments)
                finally:
                    self._busy = False
    ...
    async def _process_line(self, line: str, *, attachments: list[str] | None = None) -> bool:
        assert self._bundle is not None
        attachment_suffix = f" [attachments: {len(attachments or [])}]" if attachments else ""
        await self._emit(
            BackendEvent(type="transcript_item", item=TranscriptItem(role="user", text=f"{line}{attachment_suffix}"))
        )
    ...
    async def _emit(self, event: BackendEvent) -> None:
        async with self._write_lock:
            payload = _PROTOCOL_PREFIX + event.model_dump_json(ensure_ascii=True) + "\n"
